...
st.set_page_config(page_title="Real-time Face Mask Detection", page_icon="./assets/faceman_cropped.png", layout="centered")

# -------------Header Section------------------------------------------------

title = '<p style="text-align: center;font-size: 40px;font-weight: 550; "> FINAL YEAR PROJECT : Realtime Face Mask Detection</p>'
st.markdown(title, unsafe_allow_html=True)

st.markdown(
    "This project is made by Ayesha Falak and Anchal Mishra under the guidance of Dr. Veena Mittal. "
    "as a part of the academic final year project at Galgotias college of engineering and technology.")
...
with st.sidebar:
    ...
    st.markdown("## Made by **AYESHA FALAK & ANCHAL MISHRA** \U0001F609")
...
if detection_mode == "Webcam Image Capture":
    ...
    cascade2 = cv2.CascadeClassifier("haarcascade_mcs_mouth.xml")
    ...
    if img_file_buffer is not None:
        ...
        with st.spinner("Detecting faces ..."):
            ...
            # Load the cascade
            faces = cascade1.detectMultiScale(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), 1.1, 3)
            mask = cascade2.detectMultiScale(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), 1.1, 3)
            ...
            if len(faces)>0:
                for (x,y,w,h) in faces:
                    ...
                    if len(mask)>0:
                        cv2.putText(imghdr,"No Mask",(x,y),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
                ...
                cv2.putText(img,"Checking..",(280,260),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2)
    ...
    with st.spinner(spinner_message):
        ...
        class VideoProcessor:
            ...
            def recv(self, frame):
                ...
                # detect faces
                faces = cascade1.detectMultiScale(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY), 1.1, 3)
                mask = cascade2.detectMultiScale(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY), 1.1, 3)
